...
fn test_depth_0() {
    ...
    assert!(matches!(mt.verify("hello", 1),
        Err(Error::NoSuchleaf {
            index: 1
        })
    ));
    assert!(!mt.verify("world", 0).unwrap());
    assert!(matches!(mt.verify("world", 1),
        Err(Error::NoSuchleaf {
            index: 1
        })
    ));
...
fn test_depth_4() {
    const CAP: usize = 31;
    let data = [
        "we", "ert", "dsf", "ewfc", "dsfassdaf", "fdsqwwefew", "eegq3rre", "regfsd43",
        "d43grt43r", "rv452fgre", "3214rgr25143t2rewg23453rgwr234521gt143t2g", "r3fwf22d",
        "rqf32534g5312f2f53gf12dere", "32rtegwf", "3rf4f43rf", "j875um78loj6ki7t"
    ];
    let mt: MerkleTree<DigestWrap, Sha256Context, CAP> =
        MerkleTree::from_slice(&data).unwrap();
